...
function unit_card_obj:create_land_unit_card_for_frontend(parent)
    ...
    for i = 0, parent:ChildCount() - 1 do
        ...
        if child:Id() == "land_unit_card_"..self._unit_key then
            ...
            UTILITY.remove_component(child)
        end
    end
            ...
            function(context)
                ...
                tm:callback(function()
                    core:add_listener(
                        ...
                        "ComponentMouseOn",
                        function(context) 
                            return UIComponent(context.component) ~= self._uic_on_hover and UIComponent(context.component) ~= self._stat_card_uic
                        end,
                        function(context)]]
                ...
                tm:callback(function() core:remove_listener("inner_hide_card") end, 55)]]
            end,
            false
        )
    end
...
--v method()
function unit_card_obj:reset_unit_card()
    --# assume self: VANDY_UC

    --Util.delete(self._land_card_uic)
    --Util.delete(self._stat_card_uic)
end
...
function unit_card_manager:add_frontend_unit_for_starting_general(unit_key, starting_general)
    ...
    -- hopefully prevents bugginess when you leave and go back to the frontend lord's section
    core:add_listener(
        "ResetUnitCards",
        "ComponentLClickUp",
        function(context)
            -- check if the land unit card isn't visible
            local uic = unit_card._land_card_uic
            if is_uicomponent(uic) and not uic:Visible() then
                return true
            end
            return false
        end,
        function(context)
            unit_card:reset_unit_card()
        end,
        true
    )
